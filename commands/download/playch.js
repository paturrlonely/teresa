...
import { canvas as canvasYts } from "../../library/canvas/canvas-yts.js"
import { canvas as canvasPlay } from "../../library/canvas/canvas-play.js"
    ...
    try {
        ...
        if (text?.startsWith("res|")) {
            ...
            try {
                ...
                let apiUrl =
                    `https://ytdlpyton.nvlgroup.my.id/download/audio?url=${encodeURIComponent(url)}&mode=url&bitrate=${bitrate}k`
                ...
                let data = await fetch(apiUrl, { headers }).then(v => v.json())
                ...
                await new Promise((resolve, reject) => {
                    ffmpeg(tmpMp3)
                        ...
                        .toFormat("opus")
                        .on("start", cmd => {
                            console.log("[DEBUG] FFmpeg start:", cmd)
                        })
                        .on("end", () => {
                ...
                console.log("[DEBUG] Audio berhasil dikirim")
                await m.reply("✅ Audio berhasil dikirim ke *Channel*")

            } catch (e) {
                console.error("[DEBUG] ERROR di mode res:", e)
                m.reply("❌ Error saat memproses audio")
            } finally {
                await global.loading(m, conn, true)
                if (fs.existsSync(tmpMp3)) {
                    fs.unlinkSync(tmpMp3)
                    console.log("[DEBUG] tmpMp3 dihapus")
                }
                if (fs.existsSync(tmpOpus)) {
                    fs.unlinkSync(tmpOpus)
                    console.log("[DEBUG] tmpOpus dihapus")
                }
            }
            return
        }
        ...
        if (/^https?:\/\//.test(text)) {
            ...
            return await conn.sendMessage(m.chat, {
                ...
                buttons: [{
                    ...
                    buttonText: { displayText: "🎵 Pilih Bitrate" },
                    type: 4,
                    nativeFlowInfo: {
                        name: "single_select",
        ...
        await conn.sendMessage(m.chat, {
            ...
            buttons: [{
                ...
                buttonText: { displayText: "📥 Pilih Audio" },
                type: 4,
                nativeFlowInfo: {
                    name: "single_select",
                    paramsJson: JSON.stringify({
